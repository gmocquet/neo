...
| `neo-pr-feedbacks-review`      | Developers        | Closes the review loop of an open pull request: collects every comment (inline threads, review remarks, PR comments) from colleagues and AI reviewers alike, judges each one against the project's architecture, specs, rules and ecosystem, and sorts it into `accepted`, `nice-to-have`, `out-of-scope`, `context-gap` or `not-applicable`. Shows the verdict table, waits for the go-ahead, lands each accepted point in a dedicated commit, pushes, replies with clickable commit links, and resolves every thread that has no pending question. `--mode rush\|polish` decides the nice-to-haves. |
